...
def create_model(model_type='efficientnet', in_dim=1, out_dim=1, use_pretrained=True, include_final=True):
    ...
    if model_type == 'vit':
        ...
        load_weights = None
        if use_pretrained:
            load_weights = MobileNet_V2_Weights.IMAGENET1K_V2
        backbone = mobilenet_v2(weights=load_weights)
        if in_dim != 3:
            backbone.features[0][0] = nn.Conv2d(in_dim, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        if include_final:
            # Create new classifier while preserving dropout and other layers
            backbone.classifier = nn.Sequential(
                nn.Dropout(p=0.2),
                nn.Linear(in_features=1280, out_features=out_dim, bias=True)
            )
        else:
            backbone.classifier[1] = nn.Identity()        

    elif model_type == 'mobilevit':
        backbone = timm.create_model('mobilevit_v2_100', pretrained=True)  # use timm instead
        if in_dim != 3:
            backbone.stem[0] = nn.Conv2d(in_dim, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        if include_final:
            backbone.head = nn.Linear(in_features=512, out_features=out_dim, bias=True)
        else:
        ...
        load_weights = None
        if use_pretrained:
            load_weights = Swin_T_Weights.IMAGENET1K_V1
        backbone = swin_t(weights=load_weights)
        if in_dim != 3:
            backbone.features[0][0] = nn.Conv2d(in_dim, 96, kernel_size=(4, 4), stride=(4, 4), padding=(0, 0), bias=False)
        if include_final:
            backbone.head = nn.Linear(in_features=768, out_features=out_dim, bias=True)
        else:
            backbone.head = nn.Identity()
            
    elif model_type == 'levit':
        backbone = timm.create_model('levit_256', pretrained=use_pretrained)
        if in_dim != 3:
            # Modify the stem conv layer for different input channels
            backbone.stem_conv = nn.Conv2d(in_dim, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
        if include_final:
            backbone.head = nn.Linear(in_features=512, out_features=out_dim, bias=True)
        else:
            backbone.head = nn.Identity()
    
    elif model_type == 'deit_small':
        backbone = timm.create_model('deit_small_patch16_224', pretrained=True)
        if in_dim != 3:
            backbone.patch_embed.proj = nn.Conv2d(in_dim, 384, kernel_size=(16, 16), stride=(16, 16))
        if include_final:
            backbone.head = nn.Linear(in_features=384, out_features=out_dim, bias=True)
        else:
            backbone.head = nn.Identity() 
